...
/**
 * Published to the Spring event bus at the end of any
 * service method that mutates state.
 *-
 * The AuditEventListener picks this up AFTER the originating
 * transaction commits (phase = AFTER_COMMIT), so:
 *   Only successful ops are recorded
 *   Rolled-back transactions leave zero phantom audit rows
 *   Audit write failure doesn't roll back the business tx
 */
@Getter
